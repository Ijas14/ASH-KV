...
class DitheredINT2Codec:
    ...
    def encode(self, source_bytes: bytes) -> bytes:
        ...
        # 3. Stochastic Dithering (Fixed Seed)
        torch.manual_seed(42)
        noise = (torch.rand_like(arr_inliers) - 0.5) * delta
        
        scaled = (arr_inliers + noise) / delta + 1.5
        rounded = scaled.round().clamp(0, 3).to(torch.uint8)
        
        # 4. Bit-Packing (4 values per byte)
        rounded_flat = rounded.view(-1)
        v0 = rounded_flat[0::4]
        v1 = rounded_flat[1::4]
        v2 = rounded_flat[2::4]
        v3 = rounded_flat[3::4]
    ...
    def decode(self, target_bytes: bytes) -> bytes:
        ...
        dequantized_flat = dequantized.view(-1)
